...
def test_plugin_with_no_config_gets_empty_dict():
    """A plugin whose name isn't listed under ``[plugins]`` still
    receives a context — just with an empty config dict."""
    captured: list[PluginContext] = []
...
def test_post_plan_aggregates_violations_across_plugins():
    ...
    out = run_post_plan(_FAKE_CONFIG, _FAKE_REPO, _FAKE_PLAN, registry=PluginRegistry([A(), B()]))
...
def test_enrich_changelog_aggregates_per_component():
    ...
    out = run_enrich_changelog(
        _FAKE_CONFIG, _FAKE_REPO, _FAKE_PLAN, "api", registry=PluginRegistry([P()])
    )
    assert len(out) == 1
...
def test_status_lines_concatenates_all_plugins():
    ...
    out = run_status_lines(_FAKE_CONFIG, _FAKE_REPO, _FAKE_PLAN, registry=PluginRegistry([A(), B()]))
...
def test_config_without_plugins_attribute_is_handled():
    ...
    crash the runner — we fall back to an empty plugins map."""
    captured: list[PluginContext] = []
